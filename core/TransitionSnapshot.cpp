    ...
    for (const auto& ts : m_tracks)
    {
        ...
        if (!ts.fxChainChunk.empty())
        {
            ...
            while (*p)
            {
                ...
                if (len > 0)
                {
                    // Use std::string — avoids 4095-byte truncation of long
                    // VST3/AU preset base64 blobs that would corrupt the chunk.
                    std::string linebuf(p, len);
                    ctx->AddLine("%s", linebuf.c_str());
                }
                p = (*nl == '\n') ? nl + 1 : nl;
            }
                                                     ...
                                                     ProjectStateContext* ctx)
{
    ...
    while (ctx->GetLine(line, sizeof(line)) == 0)
    {
        ...
        else if (strncmp(trimmed, "FX ", 3) == 0)
        {
            ...
            curFX.enabled = (en != 0);
            // Guard against corrupt / malicious files: large paramCount
            // would cause std::bad_alloc and terminate REAPER.
            if (curFX.paramCount < 0 || curFX.paramCount > 4096)
                curFX.paramCount = 0;
