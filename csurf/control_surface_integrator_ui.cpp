...
static WDL_DLGRET dlgProcEditAdvanced(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_CLOSE:
            SendMessage(hwndDlg, WM_USER+1024, 0, 0);
...
static bool s_surfaceMotorizedFaders = false;
static bool s_surfaceMotorizedFadersInferTouch = false;
...
struct SurfaceLine
{
    ...
    bool motorizedFaders = false;
    bool inferTouch = false;
    
    SurfaceLine()  {}
        
    SurfaceLine(const char * const aType, string aName, int aChannelCount, int anInPort, int anOutPort, int aRefreshRate, int maxSysExMessages, bool aMotorizedFaders = false, bool aInferTouch = false) : type(aType), name(aName), channelCount(aChannelCount), inPort(anInPort), outPort(anOutPort), surfaceRefreshRate(aRefreshRate), surfaceMaxSysExMessagesPerRun(maxSysExMessages), motorizedFaders(aMotorizedFaders), inferTouch(aInferTouch) {}
...
static WDL_DLGRET dlgProcPageSurface(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            filesystem::path path { string(GetResourcePath()) + "/LiveTools/Surfaces"};
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            if (s_editMode)
            {
                ...
                CheckDlgButton(hwndDlg, IDC_CHECK_MotorizedFaders, s_surfaceMotorizedFaders ? BST_CHECKED : BST_UNCHECKED);
                CheckDlgButton(hwndDlg, IDC_CHECK_InferTouch, s_surfaceMotorizedFadersInferTouch ? BST_CHECKED : BST_UNCHECKED);
            }
            else
            {
                ...
                CheckDlgButton(hwndDlg, IDC_CHECK_MotorizedFaders, BST_UNCHECKED);
                CheckDlgButton(hwndDlg, IDC_CHECK_InferTouch, BST_UNCHECKED);
            }
        }
            break;
            ...
            switch(LOWORD(wParam))
            {
                case IDOK:
                    if (HIWORD(wParam) == BN_CLICKED)
                    {
                        ...
                        s_surfaceMotorizedFaders = (IsDlgButtonChecked(hwndDlg, IDC_CHECK_MotorizedFaders) == BST_CHECKED);
                        s_surfaceMotorizedFadersInferTouch = (IsDlgButtonChecked(hwndDlg, IDC_CHECK_InferTouch) == BST_CHECKED);
...
WDL_DLGRET dlgProcMainConfig(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
                ...
                switch(LOWORD(wParam))
                {
                    ...
                    case IDC_BUTTON_AddMidiSurface:
                        if (HIWORD(wParam) == BN_CLICKED)
                        {
                            s_dlgResult = false;
                            s_editMode = false;
                            s_surfaceMotorizedFaders = false;
                            s_surfaceMotorizedFadersInferTouch = false;
                            DialogBox(g_hInst, MAKEINTRESOURCE(IDD_DIALOG_MidiSurface), hwndDlg, dlgProcMidiSurface);
                            if (s_dlgResult == IDOK)
                            {
                                s_surfaces.push_back(make_unique<SurfaceLine>(s_MidiSurfaceToken, s_surfaceName, s_surfaceChannelCount,                                                                       s_surfaceInPort, s_surfaceOutPort, s_surfaceRefreshRate, s_surfaceMaxSysExMessagesPerRun, s_surfaceMotorizedFaders, s_surfaceMotorizedFadersInferTouch ));
                    ...
                    case IDC_BUTTON_EditSurface:
                        if (HIWORD(wParam) == BN_CLICKED)
                        {
                            ...
                            if (index >= 0)
                            {
                                ...
                                s_surfaceMotorizedFaders = s_surfaces[index]->motorizedFaders;
                                s_surfaceMotorizedFadersInferTouch = s_surfaces[index]->inferTouch;
                                ...
                                if (s_dlgResult == IDOK)
                                {
                                    ...
                                    s_surfaces[index]->motorizedFaders = s_surfaceMotorizedFaders;
                                    s_surfaces[index]->inferTouch = s_surfaceMotorizedFadersInferTouch;
        ...
        case WM_INITDIALOG:
        {
            ...
            string iniFilePath = string(GetResourcePath()) + "/LiveTools/LiveTools.ini";
            ...
            for (string line; getline(iniFile, line) ; )
            {
                ...
                if (lineNumber == 1)
                {
                    ...
                    if (versionProp)
                    {
                        ...
                        {
                            char tmp[MEDBUF];
                            snprintf(tmp, sizeof(tmp), __LOCALIZE_VERFMT("Version mismatch -- Your LiveTools.ini file is not %s.","csi_mbox"), s_MajorVersionToken);
                        ...
                        if (const char *surfaceNameProp = pList.get_prop(PropertyType_SurfaceName))
                        {
                            if (const char *surfaceChannelCountProp = pList.get_prop(PropertyType_SurfaceChannelCount))
                            {
                                if ( ! strcmp(surfaceTypeProp, s_MidiSurfaceToken) && tokens.size() >= 7)
                                {
                                    ...
                                    {
                                        bool motorizedFaders = pList.get_prop(PropertyType_MotorizedFaders) != NULL && atoi(pList.get_prop(PropertyType_MotorizedFaders)) != 0;
                                        bool inferTouch = pList.get_prop(PropertyType_MotorizedFadersInferTouch) != NULL && atoi(pList.get_prop(PropertyType_MotorizedFadersInferTouch)) != 0;
                                        s_surfaces.push_back(make_unique<SurfaceLine>(surfaceTypeProp, surfaceNameProp, atoi(surfaceChannelCountProp),                                                                       atoi(pList.get_prop(PropertyType_MidiInput)), atoi(pList.get_prop(PropertyType_MidiOutput)), atoi(pList.get_prop(PropertyType_MIDISurfaceRefreshRate)), atoi(pList.get_prop(PropertyType_MaxMIDIMesssagesPerRun)), motorizedFaders, inferTouch));
        ...
        case WM_USER+1024:
        {
            FILE *iniFile = fopenUTF8((string(GetResourcePath()) + "/LiveTools/LiveTools.ini").c_str(), "wb");
            ...
            if (iniFile)
            {
                ...
                for (auto &surface : s_surfaces)
                {
                    ...
                    if (type == s_MidiSurfaceToken)
                    {
                        ...
                        fprintf(iniFile, "%s=%d ", plist.string_from_prop(PropertyType_MotorizedFaders), (int)surface->motorizedFaders);
                        fprintf(iniFile, "%s=%d ", plist.string_from_prop(PropertyType_MotorizedFadersInferTouch), (int)surface->inferTouch);
                    }
