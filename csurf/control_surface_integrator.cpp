...
CSurfIntegrator *g_csi_ = nullptr;
...
static midi_Input *GetMidiInputForPort(int inputPort)
{
    ...
    {
        LogToConsole(256, "[ERROR] LiveTools: Failed to open MIDI Input port %d\n", inputPort);
    }
...
static midi_Output *GetMidiOutputForPort(int outputPort)
{
    ...
    {
        LogToConsole(256, "[ERROR] LiveTools: Failed to open MIDI Output port %d\n", outputPort);
    }
...
void CSurfIntegrator::Init()
{
    ...
    string CSIFolderPath = string(GetResourcePath()) + "/LiveTools";
    
    if ( ! filesystem::exists(CSIFolderPath))
        filesystem::create_directories(CSIFolderPath);
                        ...
                        if (const char *channelCountProp = pList.get_prop(PropertyType_SurfaceChannelCount))
                        {
                            ...
                            if ( ! strcmp(typeProp, s_MidiSurfaceToken) && tokens.size() >= 7)
                            {
                                    ...
                                    pList.get_prop(PropertyType_MaxMIDIMesssagesPerRun) != NULL)
                                {
                                    ...
                                    auto io = make_unique<Midi_ControlSurfaceIO>(this, nameProp, channelCount, GetMidiInputForPort(midiIn), GetMidiOutputForPort(midiOut), surfaceRefreshRate, maxMIDIMesssagesPerRun);
                                    io->SetMotorizedFaders(motorizedFaders);
                                    io->SetInferTouch(inferTouch);
                                    midiSurfacesIO_.push_back(std::move(io));
                                }
                            }
                        }
                    }
                    ...
                    if (const char *surfaceProp = pList.get_prop(PropertyType_Surface))
                    {
                        if (const char *surfaceFolderProp = pList.get_prop(PropertyType_SurfaceFolder))
                        {
                            if (const char *startChannelProp = pList.get_prop(PropertyType_StartChannel))
                            {
                                ...
                                string baseDir = string(GetResourcePath()) + string("/LiveTools/Surfaces/");
                                ...
                                if (!foundIt)
                                    LogToConsole(256, "[ERROR] LiveTools: Surface '%s' referenced in Page '%s' was not found in registered surfaces\n", surfaceProp, currentPage->GetName());
...
void ZoneManager::PreProcessZones()
{
    ...
    {
        char tmp[2048];
        snprintf(tmp, sizeof(tmp), __LOCALIZE_VERFMT("Please check your LiveTools.ini, cannot find Zone folder for %s in:\r\n\r\n%s/LiveTools/Zones/","csi_mbox"), GetSurface()->GetName(), GetResourcePath());
...
void TrackNavigationManager::RebuildTracks()
{
    ...
    int activeLayer = LayersEngine::Get().GetActiveLayer();
    if (activeLayer >= 0)
    {
        const LayerDef &layer = LayersEngine::Get().GetLayer(activeLayer);
        int n = CSurf_NumTracks(followMCP_);
        for (const auto &lt : layer.tracks)
        {
            if (lt.isSpacer) continue;
            for (int i = 1; i <= n; ++i)
            {
                MediaTrack *tr = CSurf_TrackFromID(i, followMCP_);
                if (tr)
                {
                    GUID *tg = GetTrackGUID(tr);
                    if (tg && memcmp(&lt.guid, tg, sizeof(GUID)) == 0)
                    {
    ...
    {
        for (int i = oldTracksSize; i > (int)tracks_.size(); i--)
...
// CSurfIntegrator
////////////////////////////////////////////////////////////////////////////////////////////////////////
static const char * const Control_Surface_Integrator = "Live Tools - Control Surfaces";
...
void CSurfIntegrator::RunDiagnostics()
{
    LogToConsole(256, "\n=== Live Tools: Surface Diagnostics ===\n");
    
    LogToConsole(256, "Registered MIDI Surfaces (%d):\n", (int)midiSurfacesIO_.size());
    for (auto &io : midiSurfacesIO_)
    {
        LogToConsole(512, "  '%s'  In: %s  Out: %s  Channels: %d\n",
            io->GetName(),
            io->HasMidiInput()  ? "OK" : "FAILED (check MIDI port number)",
            io->HasMidiOutput() ? "OK" : "FAILED (check MIDI port number)",
            io->GetChannelCount());
    }
    
    LogToConsole(256, "Pages (%d):\n", (int)pages_.size());
    for (auto &page : pages_)
    {
        auto &surfaces = page->GetSurfaces();
        LogToConsole(256, "  Page '%s': %d surface(s) loaded\n", page->GetName(), (int)surfaces.size());
        for (auto &surface : surfaces)
        {
            LogToConsole(512, "    '%s'  StartChannel: %d  Channels: %d\n",
                surface->GetName(),
                surface->GetChannelOffset(),
                surface->GetNumChannels());
        }
    }
