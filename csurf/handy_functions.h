...
// Function pointer set by SurfaceMonitorWnd when the window is visible.
// LogToConsole calls this so all control surface log output appears in the window.
extern void (*g_surfaceMonitorAppend)(const char*);
...
static void LogToConsole(int size, const char* format, Args... args)
{
    ...
    if (g_surfaceMonitorAppend) g_surfaceMonitorAppend(buffer.data());
#ifdef _DEBUG
    ofstream logFile(string(GetResourcePath()) + "/LiveTools/LiveTools.log", ios::app);
    if (logFile.is_open())
    {
        char timeStr[32];
        time_t rawtime;
