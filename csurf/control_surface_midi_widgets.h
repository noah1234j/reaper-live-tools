...
public:
    ...
    virtual void ProcessMidiMessage(const MIDI_event_ex_t *midiMessage) override
    {
        ...
        static_cast<Midi_ControlSurface *>(widget_->GetSurface())->RecordFaderMove(widget_->GetChannelNumber());
    }
};
...
public:
    ...
    virtual void SetValue(const PropertyList &properties, double value) override
    {
        if (surface_->GetMotorizedFaders())
        {
            int ch = widget_->GetChannelNumber();
            if (surface_->GetIsChannelTouched(ch) ||
                (surface_->GetInferTouch() && surface_->IsChannelInferredTouched(ch)))
            {
                LogToConsole(256, "[Touch Debug] Motor suppressed Ch%d\n", ch);
                return;
            }
        }
