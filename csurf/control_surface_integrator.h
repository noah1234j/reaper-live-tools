...
private:
    ...
    vector<ChannelLastMove> channelLastMoves_;
...
protected:
    ...
    ControlSurface(CSurfIntegrator *const csi, Page *page, const string &name, int numChannels, int channelOffset) : csi_(csi), page_(page), name_(name), numChannels_(numChannels), channelOffset_(channelOffset), modifierManager_(make_unique<ModifierManager>(csi_, (Page *)NULL, this))
    {
        ...
        for (int i = 1 ; i <= numChannels; ++i)
        {
            ...
            ChannelLastMove channelLastMove;
            channelLastMove.channelNum = i;
            channelLastMoves_.push_back(channelLastMove);
        }
    }
    ...
    void TouchChannel(int channelNum, bool isTouched)
    {
        for (auto &channelTouch : channelTouches_)
            if (channelTouch.channelNum == channelNum)
            {
                ...
                LogToConsole(256, "[Touch Debug] Surface '%s' Ch%d: %s\n", name_.c_str(), channelNum, isTouched ? "TOUCHED" : "RELEASED");
                break;
            }
    }

    void RecordFaderMove(int channelNum)
    {
        for (auto &entry : channelLastMoves_)
            if (entry.channelNum == channelNum)
            {
                entry.lastMoveTime = GetTickCount();
                return;
            }
    }

    bool IsChannelInferredTouched(int channelNum)
    {
        for (auto &entry : channelLastMoves_)
            if (entry.channelNum == channelNum)
                return (GetTickCount() - entry.lastMoveTime) < 500;
        return false;
    }
...
public:
    ...
    bool motorizedFaders_ = false;
    bool inferTouch_ = false;

    const char *GetName() { return name_.c_str(); }
    
    const int GetChannelCount() { return channelCount_; }
    bool HasMidiInput() const { return midiInput_ != nullptr; }
    bool HasMidiOutput() const { return midiOutput_ != nullptr; }
    void SetMotorizedFaders(bool v) { motorizedFaders_ = v; }
    bool GetMotorizedFaders() const { return motorizedFaders_; }
    void SetInferTouch(bool v) { inferTouch_ = v; }
    bool GetInferTouch() const { return inferTouch_; }
...
public:
    ...
    bool GetMotorizedFaders() const
    {
        return surfaceIO_->GetMotorizedFaders();
    }

    bool GetInferTouch() const
    {
        return surfaceIO_->GetInferTouch();
    }
        ...
        if (g_fxParamsWrite)
        {
            ...
            for (int i = 0; i < TrackFX_GetCount(track); ++i)
            {
                ...
                if (g_fxParamsWrite)
                {
                    ...
                    fxFile = fopenUTF8((string(GetResourcePath()) + "/LiveTools/ZoneRawFXFiles/" + fxNameNoBadChars + ".txt").c_str(), "wb");
